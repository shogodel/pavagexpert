...
import TermsAcceptanceBanner from "@/components/terms-acceptance-banner";
...
export default function ContractorDashboard() {
  ...
  const [profile, setProfile] = useState<{ company: string; email: string; phone: string; termsVersion: string } | null>(null);
  ...
  useEffect(() => {
    fetch("/api/contractor/profile")
      ...
      .then((d) => {
        if (d.ok) {
          setProfile(d.data);
          setTermsAccepted(d.data.termsVersion === "2025-01");
        } else {
    ...
    <>
      {!termsAccepted && profile && (
        <TermsAcceptanceBanner
          currentVersion={profile.termsVersion}
