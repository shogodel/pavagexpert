      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {steps.map((step, i) => (
            <motion.div
              key={step}
              className="bg-white rounded-2xl p-6 sm:p-8 shadow-lg hover:shadow-xl transition-all duration-300 border border-stone-100"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: i * 0.1 }}
            >
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-terracotta/10 to-terracotta/5 flex items-center justify-center mx-auto mb-4 shadow-inner">
              ...
              <h3 className="text-lg font-heading font-semibold text-stone-800 mb-2">{t(`${step}_title`)}</h3>
              <p className="text-sm md:text-base text-stone-500 leading-relaxed max-w-xs mx-auto">{t(`${step}_desc`)}</p>
            </motion.div>
          ))}
        </div>
      </div>
